...
class SafetyManager:
    ...
    def _check_pump_timeouts(self) -> None:
        ...
        for pin in expired_pumps:
            logger.critical(f"SAFETY: Force stopping pump on pin {pin} - timeout exceeded")
            try:
    ...
    def get_safety_status(self) -> Dict:
        ...
        return {
            ...
            "watchdog_time_remaining": max(0, self.watchdog_timeout - (time.time() - self.last_watchdog_reset)),
        }